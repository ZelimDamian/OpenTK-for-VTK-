	...
	public class Drawable : VTKObject
	{
		...
		public virtual void SetVBOs(Mesh curMesh, Shader shader)
		{
				...
				if (normalIndex != -1)
				{
					...
					GL.BindBuffer(All.ArrayBuffer, curMesh.NormalVboHandle);
					GL.VertexAttribPointer(normalIndex, 3, All.VertexAttribArrayType, true, Vector3.SizeInBytes, 0);
				}
				
				if (positionIndex != -1)
				{
					GL.EnableVertexAttribArray(positionIndex);
					GL.BindBuffer(All.ArrayBuffer, curMesh.PositionVboHandle);
					GL.VertexAttribPointer(positionIndex, 3, All.VertexAttribPointerType, true, Vector3.SizeInBytes, 0);
				}
				
				if (tangentIndex != -1)
				{
					GL.EnableVertexAttribArray(tangentIndex);
					GL.BindBuffer(All.ArrayBuffer, curMesh.TangentVboHandle);
					GL.VertexAttribPointer(tangentIndex, 3, All.VertexAttribPointerType, true, Vector3.SizeInBytes, 0);
				}
				
				if (textureIndex != -1)
				{
					GL.EnableVertexAttribArray(textureIndex);
					GL.BindBuffer(All.ArrayBuffer, curMesh.TextureVboHandle);
					GL.VertexAttribPointer(textureIndex, 2, VertexAttribPointerType.Float, true, Vector2.SizeInBytes, 0);
				}
				
				
				GL.BindBuffer(All.ElementArrayBuffer, curMesh.ElementsVBOHandle);
		}
